...
def get_market_snapshot(symbol):
    ...
    try:
        ...
        strong_accumulation = (
            is_sideway and near_lower_range and close >= ma20 * 0.995 and
            rsi >= 45 and macd_hist >= -1.0 and not dead_market and
            not post_drop_sideway and trend_bias != "bearish"
        )
        support_bounce = (
            near_lower_range and close >= support and close >= ma20 * 0.99 and
            rsi >= 45 and macd >= signal * 0.9 and trend_bias != "bearish"
        )
        ...
        if setup == "SIDEWAY ACCUMULATION PREPARE":
            ...
            structure += 12; reasons.append("breakout sudah jalan, tunggu retest")
        ...
        v_status, v_reason = validation_status(close, bid_low, bid_high, trigger, invalidation, fake_breakout, setup, volume_score, range_position, trend_bias)
        if v_status == "INVALID":
            return None
        ...
        return {
            "symbol": symbol.upper(), "score": score, "setup": setup, "close": round(close, 2),
            "change_pct": round(change_pct, 2), "volume": volume_label, "status": v_status,
            "validation": v_reason, "timing": timing, "timing_reason": timing_reason,
            "bid_low": round(bid_low, 2), "bid_high": round(bid_high, 2), "trigger": trigger,
            "invalidation": invalidation, "tp1": tp1, "tp2": tp2,
            "reason": ", ".join(reasons[:3]) if reasons else "belum ada alasan kuat",
            "tech_summary": ", ".join(tech_notes[:4]), "confidence": confidence
        }
    except Exception:
        return None
...
def format_new_candidate(data):
    lines = ["PRIORITAS BARU", "", data["symbol"], f"Status: {decision_status(data)}",
             f"Setup: {data['setup']}", f"Confidence: {data['confidence']}",
             f"Harga: {data['close']:.2f} ({data['change_pct']:+.2f}%)",
             f"Bid Zone: {data['bid_low']:.2f} - {data['bid_high']:.2f}",
             f"Trigger: {data['trigger']:.2f}", f"Invalidation: {data['invalidation']:.2f}",
...
def format_weakening_candidate(prev, new):
    return "\n".join(["WARNING", "", new["symbol"], f"Status berubah: {prev.get('decision_status','-')} -> {decision_status(new)}",
                      f"Harga sekarang: {new['close']:.2f}", f"Alasan: {new.get('validation','kualitas menurun')}"])
...
def handle_command(chat_id, text):
    ...
    if cmd == "/start":
        send_message(chat_id, "Entry Bot V16 context-aware aktif.\n\nCommand:\n/watchlist\n/scan\n/autoscanon\n/autoscanoff\n/statusauto\n/statuskandidat\n/reloadwatchlist")
        return
    if cmd == "/watchlist":
    ...
    if cmd == "/autoscanon":
        state["autoscan"] = True
        save_state()
        send_message(chat_id, "Autoscan V16 diaktifkan. Scan setiap 5 menit, notifikasi hanya saat ada perubahan penting.")
        return
    if cmd == "/autoscanoff":
        state["autoscan"] = False
        save_state()
        send_message(chat_id, "Autoscan V16 dimatikan.")
        return
    if cmd == "/statusauto":
